...
class GameField(object):
    ...
    def __init__(self,height=4,width=4,win_score=32):
        self.height=height
        self.width=width
    ...
    def draw_board(self): #draw field and fill in with data
        print('score:%d                 highscore:%d'%(self.score,self.highscore))
        line='+-------'*self.width+'+'
        row='|       '*self.width+'|'
    ...
    def get_action(self):
        action=input()
        while action not in 'wasdqr'and action!='':
            action=input()  
        return action      
